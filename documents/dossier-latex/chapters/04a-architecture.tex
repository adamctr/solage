...
J'ai réalisé \textbf{seul} l'intégralité de ce projet. Je l'ai mené selon une démarche
\textbf{itérative légère} (proche de Kanban), en deux temps : une \textbf{construction initiale}
(2024), puis une \textbf{reprise} dédiée à la sécurisation et à l'industrialisation (2026 :
conteneurisation, migration PostgreSQL, sécurité, refonte MVC). Sur un projet de cette taille,
j'ai assumé l'ensemble des rôles : planification, suivi et qualité.

\paragraph{Planification.} J'utilise le \file{README.md} du dépôt comme \textbf{tableau Kanban}
(colonnes \emph{À faire}, \emph{En cours}, \emph{À finaliser}, \emph{Terminé}) : il me sert de
\emph{backlog} de référence et hiérarchise les priorités au regard du risque par bloc de
compétences.
...
(\file{Probleme-Solution.md}) note, en quelques phrases courtes, chaque problème rencontré et la
solution retenue. Par exemple : « \emph{Migrations.php relançait les migrations à chaque requête
$\rightarrow$ cycle de requêtes inutile ; passage à un script \texttt{bin/migrate.php} lancé une
seule fois au démarrage.} » Le détail du suivi par phases figure en annexe~\ref{ann:gestion}.
